...
public class Md5Utils {
     ...
     *
     * @param password 原密码
     * @param md5psw   加密之后的密码
     * @return boolean true表示和原密码一致 false表示和原密码不一致
     */
    public static boolean getSaltverifyMD5(String password, String md5psw) {
        char[] cs1 = new char[32];
        char[] cs2 = new char[16];
        for (int i = 0; i < 48; i += 3) {
            cs1[i / 3 * 2] = md5psw.charAt(i);
            cs1[i / 3 * 2 + 1] = md5psw.charAt(i + 2);
            cs2[i / 3] = md5psw.charAt(i + 1);
        }
